...
function startOfGridMinutes() {
  return 8 * 60;
}
function endOfGridMinutes() {
  return 21 * 60;
}
...
export default function AgendaPage() {
  ...
  const loadSessionsForWeek = async (start) => {
    ...
    const data = await financeGateway.listSessionsInRange({
      start: startISO,
      end: endISO,
    });
  ...
  const events = useMemo(() => {
    const sessionEvents = buildEventsFromSessions(
      sessions,
      turmaMap,
      teacherMap,
      weekStart
    ...
    // Fallback: se não há sessões reais, mostra os slots planejados
    // a partir das meeting_rules das turmas.
    return buildEventsFromRules(turmas, teacherMap);
  }, [sessions, turmas, turmaMap, teacherMap, weekStart]);

  const classesForDay = (idx) =>
    events
      .filter((c) => c.day === idx)
  ...
  function handleEventClick(ev) {
    ...
    // rule event → abrir modal de nova aula com prefill
    const turmaId = String(ev.id || "")
      .replace(/^rule-/, "")
      .split("-")[0];
    setNewPrefill({
      turma_id: turmaId,
  ...
  const roomsCount = new Set(
    events.map((e) => e.room).filter((r) => r && r !== "—")
  ).size;
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <>
          {/* Mobile day picker (apenas no view=day) */}
          {view === "day" && (
                  ...
                  >
                    ...
                    <span
                      className={`text-base font-semibold tabular-nums ${
                        active ? "text-white" : "text-[var(--p-text)]"
                      }`}
                    >
                      {n}
                    </span>
          ...
          {view === "week" && (
            <>
              <div className="p-card hidden md:block overflow-hidden">
                <div className="grid grid-cols-[60px_repeat(6,1fr)] border-b border-[var(--p-border)] bg-[var(--p-surface-2)]">
                  ...
                  {DAYS.map((d, i) => (
                    <div
                      key={d}
                      className="px-3 py-3 border-l border-[var(--p-border)]"
                    >
                ...
                >
                  ...
                  {DAYS.map((d, dIdx) => (
                    <div
                      key={d}
                      className="relative border-l border-[var(--p-border)]"
                    >
                      {HOURS.map((h) => (
                        <div
                          key={h}
                      ...
                      {classesForDay(dIdx).map((c) => {
                        const top =
                          ((hourToMinutes(c.start) - startOfGridMinutes()) /
                            totalMin) *
                          100;
                        const height =
                          ((hourToMinutes(c.end) - hourToMinutes(c.start)) /
                            totalMin) *
                          100;
                        return (
                          <button
              ...
              {/* Mobile no view=Semana mostra lista do dia atual */}
              <div className="md:hidden">
                <DayList
                  list={classesForDay(day)}
      ...
      {list.map((c) => (
        ...
        >
          ...
          <div className="flex w-16 flex-col">
            ...
            <div className="text-xs text-[var(--p-text-faint)] tabular-nums">
              {c.end}
            </div>
          </div>
...
function NewSessionModal({ turmas, prefill, onClose, onCreated }) {
  const initialDate =
    prefill?.datetime?.slice(0, 10) || new Date().toISOString().slice(0, 10);
  const initialTime = prefill?.datetime?.slice(11, 16) || "19:00";
  const [turmaId, setTurmaId] = useState(
    prefill?.turma_id || turmas?.[0]?.id || ""
  );
  const [date, setDate] = useState(initialDate);
  const [time, setTime] = useState(initialTime);
  const [durationHours, setDurationHours] = useState(
    String(prefill?.duration_hours || 1)
  );
  ...
  return (
    <AppModal title="Nova aula" onClose={saving ? () => {} : onClose}>
...
// ─── Modal: Detalhes da aula + Presença inline ───────────────────
...
function SessionDetailsModal({
  ...
  const teacher = turma.teacher_id
    ? teacherMap[turma.teacher_id] || "—"
    : "Sem professor";
  ...
  async function togglePresence(studentId, present) {
    ...
    try {
      ...
      await financeGateway.upsertAttendance(session.id, studentId, {
        present,
        note: prev.note || null,
      });
  ...
  return (
    <AppModal title={turma.name || "Aula"} onClose={onClose} maxWidth="2xl">
        ...
        {loading || !session ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <Info
                label="Duração"
                value={`${Number(session.duration_hours || 0)}h`}
              />
            ...
            <div className="flex items-center justify-between border-t border-[var(--p-border)] pt-4">
              {confirmDelete ? (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-[var(--p-danger)]">
                    Confirmar exclusão?
                  </span>
                  <button
                    type="button"
