...
/* ==== Componente que usa useSearchParams (fica dentro de <Suspense/>) ==== */
function AssiduidadeInner() {
  ...
  async function buildReport() {
    ...
    try {
      const ts =
        turmaId === "all"
          ? await financeGateway.listTurmas()
          : (await financeGateway.listTurmas()).filter((t) => t.id === turmaId);
...
/* Evita prerender estático que falha com search params no build */
...
async function reportForTurma(turma, ym) {
  ...
  for (const list of allAtt) {
    for (const a of list) {
      const e =
        map.get(a.student_id) || { name: a.student_name_snapshot || "(Aluno)", presents: 0, absents: 0 };
    ...
    <>
      {groups.map(([tid, g]) => (
        <section key={tid} className="border rounded overflow-auto mb-6">
          <div className="px-3 py-2 text-sm bg-gray-50 font-medium">{g.name}</div>
