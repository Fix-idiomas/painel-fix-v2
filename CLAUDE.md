...
- **⚠️ Regra de ouro (auth lock):** NUNCA chame `supabase.auth.*` (`getUser`/`getSession`/`getClaims`, etc.) **diretamente dentro** de um callback de `supabase.auth.onAuthStateChange`. O callback roda segurando o cadeado interno do token; chamar um método de auth que também precisa desse cadeado causa **deadlock** no `TOKEN_REFRESHED` (sessão trava em "Preparando sessão…", e leituras/escritas caem junto). **Sempre adie** com `setTimeout(() => { ... }, 0)` para liberar o cadeado antes. Vale para `SessionContext.jsx` e `src/lib/subscription.js` (`useSubscription`) — e qualquer novo assinante de `onAuthStateChange`. Testes automatizados NÃO pegam isso (é timing de runtime); validar no navegador com sessão real ao longo do tempo.
